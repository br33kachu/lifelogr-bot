...
public class Notifier extends Observer {
    ...
    public void onInform(User user, TrackingObject trackingObject) {
        ...
        if (this.trackingObjectType != TrackingObjectType.UNBEKANNT && this.recommendationController.recommendationNeeded(user, trackingObjectType)) {
            String[] result = this.recommendationController.getRecommendations(user, trackingObjectType);
